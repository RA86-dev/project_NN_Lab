...
This was made for Stardance Hackclub (*2026-2027*). (NOTE: The CDN used in this project is from Stardance! if it is no longer available, please modify src/mnist.jsx and replace the link with the on from loremh below). 
...
I also recorded a video [here](README_assets/video_recording.mp4). When it makes a mistake at one, this is because this neural network is not perfect! Its supposed to show you how to do it, however, to build a production level neural network, that would require a larger model, and more data (which would lag the browser).
## Features
